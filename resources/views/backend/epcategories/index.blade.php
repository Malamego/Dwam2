...
@section('styles')
    <link rel="stylesheet" href="{{asset('backend/datatables/dataTables.bootstrap-'.GetDirection().'.min.css')}}">
@endsection

@section('content')
    <div class="row">
        <div class="col-md-12">
            <form method="post" action="{{ route('epcategories.multi_delete') }}">
                @csrf
...
@section('scripts')
    ...
    <script type="text/javascript">
        ...
        $(document).on('click', '.createBtn', function() {
            window.location = "{{ route('epcategories.create') }}";
        });
