<div class="form-body">
    ...
    {{-- Add Post's Category --}}
    <div class="form-group{{ $errors->has('epcat_id') ? ' has-error' : '' }}">
        <label class="col-md-2 control-label">{{ trans('main.epcategory') }} <span class="required"></span> </label>
        <div class="col-md-6">
            <select class="form-control select2" id="epcat_id" name="epcat_id">
              <option value="">{{ trans('main.selectECategory') }}</option>
              @foreach ($epcat as $pc)
                  <option value="{{ $pc->id }}" {{ getData($data, 'epcat_id') == $pc->id ? 'selected' : '' }}>{{ $pc->title }}</option>
              @endforeach
            </select>
            @if ($errors->has('epcat_id'))
                <span class="help-block">
                    <strong class="help-block">{{ $errors->first('epcat_id') }}</strong>
                </span>
            @endif
        </div>
    </div>
    ...
    {{-- Add Post's Taq --}}
    <div class="form-group{{ $errors->has('eptaq_id') ? ' has-error' : '' }}">
        <label class="col-md-2 control-label">{{ trans('main.eptaqs') }} <span class="required"></span> </label>
          <option value="">{{ trans('main.selecteaqsPost') }}</option>
        <div class="col-md-6">
            <select class="form-control select2" name="eptaq_id[]" id="eptaq_id" multiple>
              @foreach ($eptaq as $pt)
                  <option value="{{ $pt->id }}" {{ getData($data, 'eptaq_id') == $pt->id ? 'selected' : '' }}>{{ $pt->name }}</option>
              @endforeach
            </select>
            @if ($errors->has('eptaq_id'))
                <span class="help-block">
                    <strong class="help-block">{{ $errors->first('eptaq_id') }}</strong>
                </span>
            @endif
        </div>
    </div>
